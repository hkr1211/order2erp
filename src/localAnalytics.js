...
export function buildWorkshopBoard({ procedurePlans = [], processReports = [], materialAlerts = [], salesOrders = [], procedureLinks = [], today = new Date() } = {}) {
  ...
  const procedureOrderMatches = buildProcedureOrderMatchMap(boardPlans, normalizedOrders, procedureLinks);
...
function buildProcedureOrderMatchMap(procedures, orders, procedureLinks = []) {
...
function classifyWorkshopSection(row = {}) {
  const text = [row.work_center_name, row.procedure_name, row.product_name, row.product_model, row.product_category].filter(Boolean).join(" ");
...
function buildOrderBattleMap(rows, today) {
  const grouped = new Map();
  for (const row of rows) {
