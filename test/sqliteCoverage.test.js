...
import { tableStats } from "../src/localDb.js";
import { buildSqliteCoverage } from "../src/sqliteCoverage.js";

test("localDb exports tableStats for system coverage queries", () => {
  assert.equal(typeof tableStats, "function");
});
...
test("buildSqliteCoverage summarizes page table dependencies and missing sources", () => {
  const coverage = buildSqliteCoverage({
    tableStats: {
      ...
      erp_warehouses: { row_count: 12, latest_at: "2026-05-23T08:05:00.000Z" },
      erp_inventory_summary: { row_count: 320, latest_at: "2026-05-23T08:10:00.000Z" },
      erp_inventory_details: { row_count: 480, latest_at: "2026-05-23T08:15:00.000Z", min_date: "2026-02-20", max_date: "2026-05-23" },
      ...
      standard_risks: { row_count: 9, latest_at: "2026-05-24T03:05:00.000Z" },
  ...
  assert.match(materials.sqlite_tables, /erp_inventory_summary/);
  assert.match(materials.sqlite_tables, /erp_inventory_details/);
  assert.equal(coverage.tables.find((row) => row.table_name === "erp_sales_orders").history_status, "90天已覆盖");
  assert.equal(coverage.tables.find((row) => row.table_name === "erp_inventory_details").history_status, "90天已覆盖");
  ...
  assert.equal(coverage.summary.history_ready_tables, 4);
  ...
  assert.ok(coverage.tables.some((row) => row.table_name === "erp_inventory_summary"));
  assert.ok(coverage.tables.some((row) => row.table_name === "erp_inventory_details"));
  assert.ok(coverage.tables.some((row) => row.table_name === "erp_warehouses"));
  ...
  assert.equal(coverage.tables.find((row) => row.table_name === "standard_risks").sync_source, "standard_risks");
});
